        ...
        if depth < self.max_depth and n_samples >= self.min_samples_split and len(np.unique(now_y)) > 1:
            ...
            for i in range(n_features):
                ...
                for j in range(n_samples):
                    H[now_X[j][i]]=H.get(now_X[j][i],[0,0])
                    H[now_X[j][i]][0]+=self.loss.gradients_(y_pred=now_current_tree[j],y_true=now_y[j])
                    H[now_X[j][i]][1]+=1
                #对该特征下的特征值进行排序
                binSet = sorted(H.items(), key=lambda x: x[0], reverse=False)
                binSet = [b for b in binSet if b[1][1] >= self.min_data_bin][:self.max_bin]
               # 数据已经分完桶，开始计算信息增益
...
class BaseGradientBoosting(object):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # 当前树的预测值
        current_value=np.zeros(X.shape[0]) #这里就是初始化为0了
        ...
        for iter in range(1, self.n_estimators+1): #遍历所有的树节点
            ...
            sample_data_index,sample_weight=self.goss(X=X,
                                                      y_true=y,
                                                      y_pred=current_value)
            if len(sample_data_index)==0:
                logger.info("----------单边采样数据量为0，训练结束----------")
                break
            X,y,current_value=X[sample_data_index],y[sample_data_index],current_value[sample_data_index]
            
            # 计算负梯度
            residual= self.loss.calculate_residual(y, current_value) #计算负梯度
            
            self.trees[iter] = Tree(X, 
                                    y,
                                    residual,
                                    self.max_depth, 
            ...
            current_value=self.loss.update_f_m(X, current_value, self.trees, iter, self.learning_rate)

    def goss(self, X, y_true, y_pred, alpha=0.4, beta=0.2, random_state=20191127):
        """
        单边梯度采样
        :param X:
    ...
    def efb(self, X, y=None):
        ...
        max_conflict_count = n_samples / 2  # 用样本总数的一半作为最大冲突量
